...
import '../domain/calculators/four_zhu/four_zhu_engine.dart';
import '../domain/calculators/four_zhu/strategies/impl/day_0_boundary_strategy.dart';
import '../domain/calculators/four_zhu/strategies/impl/day_23_boundary_strategy.dart';
...
class SolarLunarDateTimeHelper {
  ...
  /// 使用 [CalculationStrategyConfigLogicModel] 计算中国日期信息。
  ///
  /// 这是 [cacluateChineseDateInfo] 的规范版本，通过数据类传入策略配置。
  static ChineseDateInfo cacluateChineseDateInfoV2(
    DateTime time,
    CalculationStrategyConfigLogicModel config,
  ) {
    // 从 config 解构参数，保持内部变量名不变
    final strategy = config.ziStrategy;
    final jieQiPrecision = config.jieQiEntryPrecision;
    final jqType = config.jieQiType;
    final phStrategy = config.phenologyStrategy;

    // 1) 将配置映射为引擎组合
    final (ZiBoundary boundary, ChildHourMode mode) = _mapZiStrategy(strategy);
    final engine = FourZhuEngine.create(
      boundary: boundary,
      childHourMode: mode,
    );
    ...
    final (String nextJieQiName, DateTime stabilizingEnd) = _nextJieQiBoundary(
      time,
    );
    ...
    bool _sameBucket(DateTime a, DateTime b) {
      ...
      switch (p) {
        ...
        case JieQiEntryPrecision.shichen:
          final (ZiBoundary boundary, ChildHourMode mode) = _mapZiStrategy(
            strategy,
          );
    ...
    final List<Phenology> candidates = Phenology.phenologyList
        .where((p) => p.jieqi == chosenJieQi)
        .toList();
    // 物候边界应用交节精度（同桶进入）
    ...
    final Phenology wuHou = candidates.isNotEmpty
        ? candidates[idx]
        : Phenology.phenologyList[WU_HOU.indexOf(
            solarDay.getPhenologyDay().getPhenology().getName(),
          )];
  ...
  /// 旧版多参数接口，内部委托给 [cacluateChineseDateInfoV2]。
  ///
  /// **已废弃**：请改用 [cacluateChineseDateInfoV2] 配合 [CalculationStrategyConfigLogicModel]。
  @Deprecated('Use cacluateChineseDateInfoV2(time, config) instead')
  static ChineseDateInfo cacluateChineseDateInfo(
    DateTime time,
    ZiShiStrategy strategy,
    JieQiEntryPrecision jieQiPrecision,
    JieQiType jqType,
    PhenologyStrategy phStrategy,
  ) {
    return cacluateChineseDateInfoV2(
      time,
      CalculationStrategyConfigLogicModel(
        ziStrategy: strategy,
        jieQiType: jqType,
        jieQiStrategy: JieQiStrategy.day, // 未使用，保持默认
        phenologyStrategy: phStrategy,
        jieQiEntryPrecision: jieQiPrecision,
      ),
    );
  ...
  // item2 月份
  // item3 日期
  /// 使用 [CalculationStrategyConfigLogicModel] 获取八字信息。
  ///
  /// 这是 [getEighthChars] 的规范版本，通过数据类传入策略配置。
  static Tuple4<EightChars, LunarDay, Phenology, JieQiInfo> getEighthCharsV2(
    DateTime time,
    CalculationStrategyConfigLogicModel config,
  ) {
    // 从 config 解构参数，保持内部变量名不变
    final ziStrategy = config.ziStrategy;
    final jieQiPrecision = config.jieQiEntryPrecision;
    final jqType = config.jieQiType;
    final phStrategy = config.phenologyStrategy;
    ...
    final solarDay = SolarDay.fromYmd(
      anchor.effectiveDateTime.year,
      anchor.effectiveDateTime.month,
      anchor.effectiveDateTime.day,
    );
    final lunarDay = solarDay.getLunarDay();
    final lunarMonth = lunarDay.getLunarMonth();
    ...
    // 节气与物候策略
    // final JieQiType jqT = jqType;
    // final PhenologyStrategy phStrategy = phStrategy;
    ...
    DateTime jieQiStartAt = jqType == JieQiType.stabilizing
        ? stabilizingStart
        : levelingStart;
    DateTime jieQiEndAt = jqType == JieQiType.stabilizing
        ? stabilizingEnd
        : levelingEnd;
    ...
    return Tuple4(
      ...
      lunarDay,
      wuHou,
      JieQiInfo(jieQi: chosenJieQi, startAt: jieQiStartAt, endAt: jieQiEndAt),
    );
  }

  /// 旧版多参数接口，内部委托给 [getEighthCharsV2]。
  ///
  /// **已废弃**：请改用 [getEighthCharsV2] 配合 [CalculationStrategyConfigLogicModel]。
  @Deprecated('Use getEighthCharsV2(time, config) instead')
  ...
  /// 便利重载：仅传入 [DateTime]，使用默认策略配置。
  ///
  /// TODO: 当前顶层调用方（UI 层）未传入策略配置，导致计算使用默认值。
  ///  未来应从 UI 层获取用户的 [CalculationStrategyConfigLogicModel] 并传入，
  ///  使子时策略、节气类型、交节精度、物候策略等可配置。
  ///  参见 [CalculationStrategyConfigLogicModel.defaultConfig]。
  static Tuple4<EightChars, LunarDay, Phenology, JieQiInfo>
  getEighthCharsWithDefault(DateTime time) {
    return getEighthCharsV2(
      time,
  ...
  /// TODO: 此方法当前未接收 [CalculationStrategyConfigLogicModel]，UI 层调用时仅传入 DateTime。
  ///  未来应增加 config 参数，使子时策略、节气类型等可由用户配置。
  static DivinationDatetimeModel calculateNormalQueryDateTimeInfo({
    ...
    final Tuple4<EightChars, LunarDay, Phenology, JieQiInfo> chineseDateInfo =
        getEighthCharsWithDefault(
          dateTime,
        ); // TODO: 未来从 UI 层传入 CalculationStrategyConfigLogicModel
  ...
  static DivinationDatetimeModel calculateRemoveDSTQueryDateTimeInfo({
    ...
    final chineseDateInfo = getEighthCharsWithDefault(
      dateTime,
    ); // TODO: 未来从 UI 层传入 CalculationStrategyConfigLogicModel
    final LunarDay lunarDay = chineseDateInfo.item2;
    final lunarMonth = lunarDay.getLunarMonth();
  ...
  static DateTime calculateMeanSolarTimeUtc(
    ...
    final Duration meanTimeOffset = Duration(
      microseconds: offsetMicroseconds.round(),
    );
  ...
  static DateTime calculateTrueSolarTimeFromLocalClockTime(
    ...
    final Duration longitudeOffsetDuration = Duration(
      microseconds: (longitudeOffsetMinutes * 60 * 1000000).round(),
    );
    ...
    final DateTime trueSolarTimeUtc = utcTime
        .add(longitudeOffset)
        .add(eotDuration);
  ...
  static tz.TZDateTime solarTimeToTZDateTime(
    SolarTime solarTime,
    String utcTimezone,
  ) {
