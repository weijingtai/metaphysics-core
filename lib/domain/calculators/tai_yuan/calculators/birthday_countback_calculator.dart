...
class BirthdayCountbackCalculator extends TaiYuanCalculator {
  ...
  TaiYuanModel calculate(
    ...
    final taiYuanModel = calculateByDays(
      birthInfo,
  ...
  TaiYuanByDaysModel calculateByDays(
    ...
    // 从出生日向前推算300天
    final conceptionDate = birthInfo.datetime.subtract(
      Duration(days: actualMatureDays),
    );

    Tuple4<EightChars, LunarDay, Phenology, JieQiInfo?> tuple4 =
        SolarLunarDateTimeHelper.getEighthChars(
          conceptionDate,
          ziStrategy,
          JieQiEntryPrecision.hour, // 默认值
          JieQiType.stabilizing, // 默认值
          PhenologyStrategy.stabilizingBased, // 默认值
        );
    // item1 为 干支
    // item2 为 农历 日期
    // item3 为 农历 节气
